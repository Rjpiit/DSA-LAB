...
void solvePocketCube(cube_t* initialCube, int * count) {
    ...
    while (queue.front) {
        ...
        if (a > 180000000) {
            printf("Unable to solve within a reasonable number of iterations.\n");
            return;
        }
...
void getInputCube(cube_t* cube) {
    printf("Enter the initial configuration of the cube (e.g., 'RBOWYG' for each face): ");

    for (int i = 0; i < dimension; i++) {
        for (int j = 0; j < dimension; j++) {
            scanf(" %c", &cube->face0[i][j]);
        }
    }

    for (int i = 0; i < dimension; i++) {
        for (int j = 0; j < dimension; j++) {
            scanf(" %c", &cube->face1[i][j]);
        }
    }

    for (int i = 0; i < dimension; i++) {
        for (int j = 0; j < dimension; j++) {
            scanf(" %c", &cube->face2[i][j]);
        }
    }

    for (int i = 0; i < dimension; i++) {
        for (int j = 0; j < dimension; j++) {
            scanf(" %c", &cube->face3[i][j]);
        }
    }

    for (int i = 0; i < dimension; i++) {
        for (int j = 0; j < dimension; j++) {
            scanf(" %c", &cube->face4[i][j]);
        }
    }

    for (int i = 0; i < dimension; i++) {
        for (int j = 0; j < dimension; j++) {
            scanf(" %c", &cube->face5[i][j]);
        }
    }
}
...
int main(int argc, char const *argv[]) {
    ...
    // Get the initial configuration as input
    getInputCube(&cube);
